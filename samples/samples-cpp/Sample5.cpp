...
public:
    ...
    virtual bool initializeSample() override
    {
        ...
        // Create the shader signature.
        {
            ...
            shaderSignatureBuilder->beginBindingBank(1);
            shaderSignatureBuilder->addBindingBankArray(AGPU_SHADER_BINDING_TYPE_SAMPLED_IMAGE, 1024);
...

        {
            ...
            auto fragmentShader = compileShaderFromFile("data/shaders/arrayTexturedFragment.glsl", AGPU_FRAGMENT_SHADER);
            printf("vertexShader %p\n", vertexShader.get());
            printf("fragmentShader %p\n", fragmentShader.get());
        ...
        // Create the shader bindings.
        {
            ...
            auto diffuseTextureView = diffuseTexture->getOrCreateFullView();
            for(int i = 0; i < 1024; ++i)
                textureBindings->bindArrayOfSampledTextureView(0, i, 1, &diffuseTextureView);
                textureBindings->bindSampledTextureView(1, diffuseTextureView);
